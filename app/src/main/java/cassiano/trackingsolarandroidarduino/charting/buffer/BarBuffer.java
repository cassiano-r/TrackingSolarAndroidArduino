...
public class BarBuffer extends AbstractBuffer<IBarDataSet> {
    ...
    public void feed(IBarDataSet data) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            BarEntry teste = data.getEntryForIndex(i);

            // calculate the x-position, depending on datasetcount
            float x = teste.getXIndex() + teste.getXIndex() * dataSetOffset + mDataSetIndex
                    + mGroupSpace * teste.getXIndex() + groupSpaceHalf;
            float y = teste.getVal();
            float [] vals = teste.getVals();
            ...
            if (!mContainsStacks || vals == null) {
                ...
                float posY = 0f;
                float negY = -teste.getNegativeSum();
                float yStart = 0f;
